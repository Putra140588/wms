...
class Mpjabatan extends CI_Controller{
	...
	function index(){
		...
		$main_page = (empty($priv)) ? 'bo/jabatan/v_index_jabatan' : 'bo/'.$priv['error'];
	...
	function column(){
		...
		$column_array = array(
				0 => 'id_jabatan',//default order sort
				1 => 'nama_jabatan',				
		);
		return $column_array;
	}
	function get_records(){
		...
		foreach ($query as $row) {
			...
			$output['data'][]=array($nomor_urut,
									$row->nama_jabatan,					
									'<button title="'.$this->config->config['delete'].'" type="button" id="delete" class="btn btn-danger btn-circle" onclick="ajaxDelete(\''.base_url('bo/'.$this->class.'/delete').'\',\''.$row->id_jabatan.'\',\'tes\')"><i class="icon-trash"></i></button>
									 <a href="'.base_url('bo/'.$this->class.'/form/'.$row->id_jabatan).'" title="'.$this->config->config['edit'].'" class="btn btn-info btn-circle"><i class="icon-edit"></i></a>'								 
			);
			$nomor_urut++;
		}
	...
	function form($id=''){
		$this->m_master->get_login();
		$view = 'bo/jabatan/v_crud_jabatan';
		if ($id !=''){
			//akses edit
			$action = 'edit';
			$data['page_title'] = "Ubah Jabatan";
			$sql = $this->m_master->get_table_filter($this->table,array('id_jabatan'=>$id));
			foreach ($sql as $row)
	...
	function proses(){
		...
		$put['nama_jabatan'] = $post['namajabatan'];			
		if ($id != ''){
			//edit proses			
			$res = $this->m_master->updatedata($this->table,$put,array('id_jabatan'=>$id));
			if ($res > 0){
				$this->session->set_flashdata('success','Ubah data jabatan berhasil.');
			...
			if ($res > 0){
				$this->session->set_flashdata('success','Tambah data jabatan berhasil.');
				redirect('bo/'.$this->class);
			}
		}
	}
	function delete(){
		...
		if (empty($priv)){
			...
			$res = $this->m_master->deletedata($this->table,array('id_jabatan'=>$val));
			if ($res){
				echo json_encode(array('error'=>0,'msg'=>'Hapus jabatan berhasil'));
			}
		}else{
