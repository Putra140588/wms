...
class Mpkategori extends CI_Controller{
	...
	function index(){
		...
		$main_page = (empty($priv)) ? 'bo/kategori/v_index_kategori' : 'bo/'.$priv['error'];
	...
	function get_records(){
		...
		$total = count($this->m_master->get_kategori());
		...
		$query = $this->m_master->get_kategori('',$this->column()[$request['column']],$request['sorting'],$request['length'],$request['start']);
		...
		foreach ($query as $row) {
			...
			$output['data'][]=array($nomor_urut,
									...
									$row->nama_kategori,	
									$row->nama_form,				
									'<button title="'.$this->config->config['delete'].'" type="button" id="delete" class="btn btn-danger btn-circle" onclick="ajaxDelete(\''.base_url('bo/'.$this->class.'/delete').'\',\''.$row->id_kategori.'\',\'tes\')"><i class="icon-trash"></i></button>
									 <a href="'.base_url('bo/'.$this->class.'/form/'.$row->id_kategori).'" title="'.$this->config->config['edit'].'" class="btn btn-info btn-circle"><i class="icon-edit"></i></a>'								 
			);
			$nomor_urut++;
		}
	...
	function form($id=''){
		$this->m_master->get_login();
		$view = 'bo/kategori/v_crud_kategori';
		if ($id !=''){
			//akses edit
			$action = 'edit';
			$data['page_title'] = "Ubah kategori";
			$sql = $this->m_master->get_table_filter($this->table,array('id_kategori'=>$id));
			foreach ($sql as $row)
		...
		$data['class'] = $this->class;		
		$data['formorder'] = $this->m_master->get_table_column('*','tb_form_order');
		$this->load->view('bo/v_header',$data);
		$this->load->view($main_page);
		$this->load->view('bo/v_footer');
	}
	function proses(){
		...
		$put['id_kategori'] = $post['idkategori'];
		$put['nama_kategori'] = $post['namakategori'];	
		$put['id_form_order'] = $post['formorder'];		
		$put['date_update'] = $this->datenow;
		$put['add_update'] = $this->addby;		
		if ($id != ''){
			//edit proses			
			$res = $this->m_master->updatedata($this->table,$put,array('id_kategori'=>$id));
			if ($res > 0){
	...
	function delete(){
		...
		if (empty($priv)){
			...
			$res = $this->m_master->deletedata($this->table,array('id_kategori'=>$val));
			if ($res){
				echo json_encode(array('error'=>0,'msg'=>'Hapus kategori berhasil'));
			}
		}else{
