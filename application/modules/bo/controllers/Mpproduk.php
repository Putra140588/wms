...
class Mpproduk extends  CI_Controller{
	...
	function get_records(){
		...
		*/
		...
		$total = count($this->m_master->get_product());
		...
		* */
		...
		$query = $this->m_master->get_product('',$this->column()[$request['column']],$request['sorting'],$request['length'],$request['start']);
	...
	function form($id='',$detail=''){
		...
		if ($id !=''){
			...
			$sql = $this->m_master->get_product(array('id_product'=>$id));
			foreach ($sql as $row)
	...
	function proses(){
		...
		$put['nama_product'] = $post['nama'];
		$put['id_kategori'] = $post['kategori'];
		$put['id_supplier'] = $post['supplier'];		
		$put['deskripsi'] = $post['deskripsi'];
		...
		if ($id != ''){
			...
			if ($res > 0){
				$this->session->set_flashdata('success','Ubah data product berhasil.');
			...
			if ($res > 0){
				$this->session->set_flashdata('success','Tambah data product berhasil.');
				redirect('bo/'.$this->class);
			}
		}
	}
	function delete(){
		...
		if (empty($priv)){
			...
			$res = $this->m_master->updatedata('tb_product',array('deleted'=>0),array('id_product'=>$val));
			if ($res){
